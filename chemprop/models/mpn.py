...
class MPNEncoder(nn.Module):
    def __init__(self, args: Namespace, atom_fdim: int, bond_fdim: int):
        ...
        self.attention = nn.MultiheadAttention(
            ...
            num_heads=args.atten_head,
            batch_first=True,  
            dropout=args.atten_dropout
        )
        

    def forward(self,mol_graph: BatchMolGraph, features_batch=None, emb_adv=None) -> torch.FloatTensor:

        f_atoms, f_bonds, a2b, b2a, b2revb, a_scope, b_scope, bonds, batch = mol_graph.get_components()
        if self.args.cuda or next(self.parameters()).is_cuda:
            f_atoms, f_bonds, a2b, b2a, b2revb, batch = (
                    f_atoms.cuda(), f_bonds.cuda(), 
                    a2b.cuda(), b2a.cuda(), b2revb.cuda(), batch.cuda())
        ...
        atom_dense, mask = to_dense_batch(atom_hiddens, batch)
        atom_dense, score = self.attention(atom_dense, atom_dense, atom_dense, key_padding_mask=~mask,
                                need_weights=True)
        atom_hiddens = atom_dense[mask] +atom_hiddens
        ...
        output = {}
        output['mol_vecs'] = mol_vecs
        output['x_emb'] = x_emb
        output['batch'] = batch
        output['sens'] = score
        return output
        # return mol_vecs, x_emb, batch  # B x H
...
class MPN(nn.Module):
    ...
    def forward(self, batch: Union[List[str], BatchMolGraph],
                features_batch: List[np.ndarray] = None, emb_adv=None) -> torch.FloatTensor:
        if not self.graph_input:  # if features only, batch won't even be used
            batch = mol2graph(batch, self.args)

        output = self.encoder.forward(batch, features_batch, emb_adv)
