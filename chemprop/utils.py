...
def compute_classification_loss(logits, targets):
    ...
    loss = loss_func(valid_logits, valid_targets)
...
def save_best_params_to_command(
    best_value,
    ...
    for key, value in best_params.items():
        ...
        if key == 'ffn_hidden_size' and value is None:

            value_to_use = best_params.get('hidden_size', args.hidden_size)
            command_lines.append(f"    --{key} {value_to_use} \\")
    ...
    if command_lines:
        ...
        command_lines[-1] = last_line.rstrip(' \\')

    
    command_lines.append(f"best_value={best_value}")
...
def compute_pearson_correlation(x, y):
    ...
    covariance = (centered_x * centered_y).sum()

    bessel_correction = torch.sqrt((centered_x**2).sum()) * torch.sqrt((centered_y**2).sum())

    correlation = covariance / (bessel_correction + 1e-8)
    return correlation
